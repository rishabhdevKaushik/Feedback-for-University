  ...
  return (
    <>
      <nav className='absolute hidden gap-[0.625rem] md:relative md:mx-10 md:mb-10 md:flex md:h-[11.125rem] xl:mr-0 xl:flex xl:flex-col xl:gap-6 xl:pr-0 xl:pt-24'>
        ...
        <div className='rounded-xl bg-white p-6 xs:mx-3 s:mx-6 md:m-0 md:w-1/3 md:p-6 lg:mx-2 lg:w-1/3 xl:m-0 xl:h-16 xl:w-full xl:min-w-64 flex items-center justify-center'>
          <button
            onClick={() => {
              localStorage.clear();
              window.location.href = '/';
            }}
            className='w-full rounded-lg bg-red px-4 py-2 text-center text-white font-semibold hover:bg-strong-red transition-colors'
          >
            Logout
          </button>
        </div>
      ...
      <nav className='flex h-[4.5rem] items-center justify-between overflow-x-hidden bg-gradient-to-tr from-gradient-blue via-gradient-purple to-gradient-red xs:px-4 s:px-6 md:absolute md:hidden'>
        <div className='flex flex-col text-white'>
          <p className='px15 font-bold tracking-[-0.19px]'>Feedback and Suggestion Box</p>
